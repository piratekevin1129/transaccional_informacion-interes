...
function getCalendarioDate(idname){
  var element = document.getElementById(idname)
	var type = element.getAttribute('type')
  var fecha_dada = ''
	if(type=='input'){
		fecha_dada = element.value
	}else{
		fecha_dada = element.getAttribute('value')
	}
  return fecha_dada
}
...
function printCalendario(data){
	...
	var dia_hoy = new Date().getUTCDate()
	var mes_hoy = new Date().getUTCMonth()+1
	var ano_hoy = new Date().getUTCFullYear()
    ...
    //chanchullo para mr jordan
    disponibles = 'all-blanco'
			...
			for(var j = 0;j<7;j++){
				if(d<=total_dias){
					...
					if(disponibles==''){
						...
						disponible = true
					}else if(disponibles=='all-blanco'){
            disponible = true
          }
          else{
						//se estan enviado datos por el metodo printcalendario
					...
					if((mes+1)==mes_hoy&&d==dia_hoy&&ano==ano_hoy){
						clase_actual = " arl_calendario_dias_dia_actual "
					}
					if(i==0){
						//primera fila
						if(j>=dia_semana){
							...
							if(d==dia_clickeado&&(mes+1)==mes_clickeado&&ano==ano_clickeado&&clickeado){
								if(disponible){
									html+='<div class="arl_calendario_dias_dia'+clase_actual+'arl_calendario_dias_dia_selected'+clase_range+'" onclick="selectDate('+mes+','+ano+','+d+','+"'"+idname+"'"+','+"'"+accion+"'"+')"><p>'+d+'</p></div>'
								}else{
									html+='<div class="arl_calendario_dias_dia'+clase_actual+clase_range+'"><p>'+d+'</p></div>'
								}
								...
								if(disponible){
									//un dia disponible
                  if(disponibles=='all-blanco'){
                    clase_actual+='arl_calendario_dias_dia_charlatan '
                  }else{
                    clase_actual+='arl_calendario_dias_dia_disponible '
                  }
									html+='<div class="arl_calendario_dias_dia'+clase_actual+clase_range+'" onclick="selectDate('+mes+','+ano+','+d+','+"'"+idname+"'"+','+"'"+accion+"'"+')"><p>'+d+'</p></div>'
								}else{
									html+='<div class="arl_calendario_dias_dia'+clase_actual+'arl_calendario_dias_dia_normal'+clase_range+'"><p>'+d+'</p></div>'
								}
							}
							d++
						}else{
						...
						if(d==dia_clickeado&&(mes+1)==mes_clickeado&&ano==ano_clickeado&&clickeado){
							if(disponible){
								...
								html+='<div class="arl_calendario_dias_dia'+clase_actual+'arl_calendario_dias_dia_selected'+clase_range+'"><p>'+d+'</p></div>'
							}
